...
<div  class="body-title" style="font-size: 40px; width: 700px; margin: 20px auto 0; font-weight: bold; color: #666666;">운동등록</div>
		...
		<c:if test="${not empty list}" >
			 <select id=list name="exerciseType" style="width: 100px; height: 35px;">
					<c:forEach var="list" items="${list}">
         				<option value="${list.exerciseType}" >${list.typeName}</option>
      				</c:forEach>
			</select>
		</c:if>
	</td>
...
<tr height="40">
	...
	<td>
		<input type="text" name="unitTime" style="width: 15%; height: 35px;" value="${dto.exerciseName}">
...
<tr height="40">
	...
	<td>
		<input type="text" name="lossCal" style="width: 15%; height: 35px;" value="${dto.exerciseName}">
...
<tr height="40">
	...
	<td>
		<textarea style="width: 98%;" rows=10; name="pic" id="content">${dto.pic}</textarea>
	</td>
</tr>
<tr height="10px;"></tr>
...
<button type="button" class="btn-default02" onclick="javascript:location.href='<%=cp%>/myclass/addexercise/list?page=${page}';">등록취소</button>
<c:if test="${mode=='update'}">
