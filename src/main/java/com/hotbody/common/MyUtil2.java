...
public class MyUtil2 {
    ...
    public String paging(int current_page, int total_page) {
        ...
        n = current_page - numPerBlock;
        if ((total_page > numPerBlock) && (currentPageSetUp > 0)) {
			sb.append("<a onclick='listPage2(1);'>처음</a>");
			sb.append("<a onclick='listPage2("+n+");'>이전</a>");
        }
        ...
        while((page <= total_page) && (page <= currentPageSetUp + numPerBlock)) {
           if(page == current_page) {
			   ...
			   sb.append("<a onclick='listPage2("+page+");' class='numBox'>"+page+"</a>");
           }
           page++;
        }
		...
		if(n>total_page) n=total_page;
        if (total_page - currentPageSetUp > numPerBlock) {
			sb.append("<a onclick='listPage2("+n+");'>다음</a>");
			sb.append("<a onclick='listPage2("+total_page+");'>끝</a>");
        }
		sb.append("</div>");
